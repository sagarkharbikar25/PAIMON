...
import { ArrowLeft, Settings, ChevronRight, CreditCard, Bell, Shield, HelpCircle, LogOut, Camera, MapPin, User } from 'lucide-react';
...
export const Profile = () => {
  ...
  const displayName = storedUser?.name || userProfile?.fullName || 'Traveller';
  const displayEmail = storedUser?.email || '';
  const displayLocation = userProfile ? `${userProfile.state || ''}, ${userProfile.country || 'India'}`.replace(/^, /, '') : 'India';

  useEffect(() => {
    apiFetch('/trips').then((d: any) => setTripCount((d.trips || d || []).length)).catch(() => {});
  }, []);
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      if (userProfile) setUserProfile({ ...userProfile, image: reader.result as string });
    };
    ...
    <div className="bg-background min-h-screen pb-32">
      ...
      <header className="px-6 pt-12 pb-8 bg-surface-container-low rounded-b-[3rem] relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-10">
          <button onClick={() => setScreen('dashboard')} className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-primary shadow-sm">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h1 className="font-headline text-xl font-bold text-primary">Profile</h1>
          <button onClick={() => setScreen('settings')} className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-primary shadow-sm">
        ...
        <div className="flex flex-col items-center">
          <div className="relative mb-6">
            <div className="w-32 h-32 rounded-[2.5rem] overflow-hidden border-4 border-white shadow-xl bg-surface-container flex items-center justify-center cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              {userProfile?.image ? (
            ...
            <button onClick={() => fileInputRef.current?.click()} className="absolute -bottom-2 -right-2 w-10 h-10 rounded-2xl bg-primary text-white flex items-center justify-center shadow-lg border-2 border-white">
              <Camera className="w-5 h-5" />
            </button>
          </div>

          <h2 className="font-headline text-3xl font-extrabold text-primary mb-1">{displayName}</h2>
          <p className="text-on-surface-variant text-sm mb-1">{displayEmail}</p>
          ...
          <div className="flex gap-4 w-full max-w-sm">
            <div className="flex-1 bg-white/50 backdrop-blur-md p-4 rounded-2xl text-center border border-white/40">
              <p className="text-2xl font-headline font-extrabold text-primary">{tripCount}</p>
              <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Trips</p>
            </div>
            <div className="flex-1 bg-white/50 backdrop-blur-md p-4 rounded-2xl text-center border border-white/40">
              <p className="text-2xl font-headline font-extrabold text-primary">—</p>
      ...
      <main className="px-6 py-10 space-y-8">
        <section className="space-y-4">
          ...
          <div className="bg-surface-container-lowest rounded-[2.5rem] overflow-hidden shadow-sm border border-outline-variant/10">
            {menuItems.map((item, idx) => (
              <button key={item.label} className={`w-full flex items-center justify-between p-6 hover:bg-surface-container-low transition-colors ${idx !== menuItems.length - 1 ? 'border-b border-outline-variant/10' : ''}`}>
        ...
        <button onClick={handleLogout} className="w-full p-6 rounded-[2rem] bg-red-50 text-red-600 font-headline font-bold flex items-center justify-center gap-3 active:scale-95 transition-all">
