...
export const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    { role: 'bot', text: "Hello Arjun! I'm your प्रvaas concierge. How can I assist with your Kyoto journey today?" }
  ]);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center p-4 sm:p-6">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-2xl bg-white rounded-[3rem] shadow-2xl overflow-hidden flex flex-col h-[80vh] max-h-[800px]"
      >
        {/* Header */}
        <header className="px-8 py-6 border-b border-outline-variant/10 flex items-center justify-between bg-white/80 backdrop-blur-xl">
          ...
          <button 
            onClick={() => setShowChatbot(false)}
            className="w-10 h-10 rounded-full bg-surface-container-low flex items-center justify-center text-primary hover:bg-surface-container transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </header>
        ...
        <main 
          ref={scrollRef}
          className="flex-grow p-8 overflow-y-auto no-scrollbar space-y-6 bg-surface-container-lowest/30"
        >
          {messages.map((msg, i) => (
            <div key={i} className={cn(
              ...
              <div className={cn(
                "max-w-[85%] p-6 rounded-[2.5rem] shadow-sm",
                msg.role === 'user' 
        ...
        <div className="p-8 bg-white border-t border-outline-variant/10">
          <div className="flex items-center gap-3 bg-surface-container-low p-3 rounded-[2.5rem] border border-outline-variant/10">
            <button className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-outline hover:text-primary transition-colors shadow-sm">
            ...
            <input 
              ...
              className="flex-grow bg-transparent border-none font-medium text-sm focus:ring-0 px-2"
            />
            <button className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-outline hover:text-primary transition-colors shadow-sm">
            ...
            <button 
              onClick={handleSend}
              disabled={!input.trim()}
              className="w-14 h-14 rounded-[1.5rem] primary-gradient text-white flex items-center justify-center shadow-lg disabled:opacity-50 transition-all active:scale-95"
            >
